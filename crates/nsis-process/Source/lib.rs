...
fn FindProcessCurrentUser() -> Result<(), Error> {
	...
	if let Some(user_sid) = get_sid(GetCurrentProcessId()) {
		if processes.into_iter().any(|pid| belongs_to_user(user_sid, pid)) {
			push(ZERO)
		} else {
			push(ONE)
		}
...
unsafe fn belongs_to_user(user_sid:*mut c_void, pid:u32) -> bool {
	...
	// Trying to get the sid of a process of another user will give us an "Access
	// Denied" error. TODO: Consider checking for HRESULT(0x80070005) if we want to
	// return true for other errors to try and kill those processes later.
